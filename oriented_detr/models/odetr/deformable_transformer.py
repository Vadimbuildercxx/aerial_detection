...
class DeformableTransformer(nn.Module):
    ...
    def get_orient_proposals(self, output_proposals, polar_length, move_sum):
        ...
        angles = torch.linspace(0, 2 * math.pi, 12 + 1, device=output_proposals.device)[:-1]
    ...
    def forward(self, srcs, masks, pos_embeds, attn_mask=None):
        ...
        enc_outputs_coord_unselected = self.get_orient_proposals(output_proposals, polar_length, center_move)
        topk = self.num_queries
        topk_proposals = torch.topk(enc_outputs_class_unselected.max(-1)[0], topk, dim=1)[1] # bs, nq

        # gather boxes
        refpoint_embed_undetach = torch.gather(enc_outputs_coord_unselected, 1, topk_proposals.unsqueeze(-1).unsqueeze(-1).repeat(1, 1, 13 ,2)) # unsigmoid
    ...
    def forward(self, tgt, memory,
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            if self.bbox_embed is not None:
                ...
                outputs_unsig_part = delta_unsig_part + reference_before_sigmoid[:, :, 0:12, :]
                delta_unsig_sum = self.bbox_embed_sum[layer_id](output[:, :, 12:13, :])
                outputs_unsig_sum = delta_unsig_sum + reference_before_sigmoid[:, :, 12:13, :]
                outputs_unsig = torch.cat([outputs_unsig_part, outputs_unsig_sum], dim=-2)
...
class DeformableTransformerDecoderLayer(nn.Module):
    ...
    def forward_ffn_sum(self, tgt):
        tgt2 = self.linear2_sum(self.dropout3_sum(self.activation_sum(self.linear1_sum(tgt))))
    ...
    def forward_ca_sum(self,
        ...
        tgt2 = self.cross_attn_sum(self.with_pos_embed(tgt, tgt_query_pos).transpose(0, 1),
                               tgt_reference_points.transpose(0, 1).contiguous(),
                               memory.transpose(0, 1), memory_spatial_shapes, memory_level_start_index, memory_key_padding_mask).transpose(0, 1)
    ...
    def forward_ca(self,
        ...
        tgt_reference_points = tgt_reference_points.permute(0, 3, 1, 2, 4).contiguous().view(-1, bs, 4, 2)
    ...
    def forward(self,
        ...
        for funcname in self.module_seq:
            if funcname == 'ffn':
                ...
                part_tgt = self.forward_ca(part_tgt, tgt_query_pos, tgt_query_sine_embed, \
                    tgt_key_padding_mask, tgt_reference_points, \
                        memory, memory_key_padding_mask, memory_level_start_index, \
                            memory_spatial_shapes, memory_pos, self_attn_mask, cross_attn_mask)
                sum_tgt = self.forward_ca_sum(sum_tgt, tgt_query_pos, tgt_query_sine_embed, \
                                              tgt_key_padding_mask, tgt_reference_points, \
                                              memory, memory_key_padding_mask, memory_level_start_index, \
                                              memory_spatial_shapes, memory_pos, self_attn_mask, cross_attn_mask)

            elif funcname == 'sa':
                sum_tgt = self.forward_sa(tgt, tgt_query_pos, tgt_query_sine_embed, \
                    tgt_key_padding_mask, tgt_reference_points, \
                        memory, memory_key_padding_mask, memory_level_start_index, \
                            memory_spatial_shapes, memory_pos, self_attn_mask, cross_attn_mask)
                part_tgt = tgt[:, :, 0:12, :]
            elif funcname == 'intra_sa':
                tgt = self.forward_intra_sa(tgt, tgt_query_pos, tgt_query_sine_embed, \
                    tgt_key_padding_mask, tgt_reference_points, \
                        memory, memory_key_padding_mask, memory_level_start_index, \
                            memory_spatial_shapes, memory_pos, self_attn_mask, cross_attn_mask)
            else:
